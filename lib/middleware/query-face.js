...
const addEndpoint = (queryName, dbName, originalQueryTemplate) => {
  if (process.env.NODE_ENV === 'development') {
    if (queryName) {
      ...
      db.select('*')
        ...
        .then(qft => {
          if (qft) {
            db.update({ query: originalQueryTemplate })
              .from('query_face_templates')
              .where('name', queryName)
              .then(result => result);
          } else {
            db.insert({ name: queryName, query: originalQueryTemplate })
    ...
    Promise.all(
      queries.map(currentQuery => {
        ...
        return executableQuery
          .then(queryResult => {
            addEndpoint(queryName, trxName, originalQueryTemplate);
...
const executeQueryDirectly = (req, res) => {
  ...
  Promise.all(
    queries.map(currentQuery => {
      ...
      return executableQuery
        .then(queryResult => {
          addEndpoint(queryName, dbName, originalQueryTemplate);
          currentQuery.queryResult = queryResult;
          return typeof queryResult === 'number'
            ? queryResult.toString()
            : queryResult;
