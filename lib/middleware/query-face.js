  ...
  if (req.beforeEventResults) {
    if (req.beforeEventResults[queryName] === false) {
      ...
      res
        .status(status)
        .send(
          QueryFaceResponse({ errors: [{ message: error, dbName, ...rest }] })
        );
      return false;
    }
  }
    ...
    return db
      ...
      .then(qft => {
        if (qft && qft.query === originalQueryTemplate) {
          throw new Error(
            'Release failed. Query you are trying to release is the same of latest version.'
          );
        }
        if (qft) {
          return db
  ...
  db.transaction(queryBuilderTrx => {
    Promise.all(
      queries.map(currentQuery => {
        ...
        const { query, queryName } = currentQuery;
      ...
      .then(results => {
        ...
        res.status(200).send(QueryFaceResponse({ results }));
      })
      .catch(err => {
        logger.error(err);
    ...
    res
      .status(400)
      .send(
        QueryFaceResponse({ errors: [{ message: err.toString(), trxName }] })
      );
  });
};
...
const executeQueryWithDependency = function*(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      let executableQuery = db.queryBuilder();
      const { query, queryName, responseAlias } = currentQuery;
      if (!Array.isArray(query)) {
        throw new Error('query must be an array');
      }
      const beforeEventResult = checkBeforeEventResult(
    ...
    if (errors.length) {
      queryBuilderTrx.rollback && queryBuilderTrx.rollback();
      res.status(400).send(QueryFaceResponse({ errors: errors }));
    } else {
      queryBuilderTrx.commit && queryBuilderTrx.commit();
      executeAfterEvents(queries, req, res);
      res.status(200).send(QueryFaceResponse({ results }));
...
const executeQueryDirectly = (req, res) => {
  ...
  return Promise.all(
    queries.map(currentQuery => {
      ...
      logger.info(
        `> executeQueryDirectly: trying to execute query: [${queryName}]`
      );
      return executableQuery
        .then(queryResult => {
          currentQuery.queryResult = queryResult;
          logger.info(`> executeQueryDirectly: successfully executed query`);
          return typeof queryResult === 'number'
            ? queryResult.toString()
            : queryResult;
        })
        .catch(err => {
          logger.error(`> executeQueryDirectly: an error occurred`, err);
          currentQuery.queryError = err;
    ...
    .then(results => {
      logger.info(`> executeQueryDirectly: executeAfterEvents is started`);
      executeAfterEvents(queries, req, res);
      res.status(200).send(
        QueryFaceResponse({
    ...
    .catch(err => {
      logger.error(`> executeQueryDirectly: an outer error occurred`, err);
      res
        .status(400)
...
const executeQuery = async (req, res, next) => {
  try {
    ...
    logger.info(`> executeBeforeEvents is started`);
    executeBeforeEvents(queries, req, res);
    if (dependent) {
      logger.info(`> executeQueryWithDependency is started`);
      runGenerator(executeQueryWithDependency(req, res, next));
    } else if (transaction) {
      logger.info(`> executeQueryWithTransaction is started`);
      executeQueryWithTransaction(req, res, next, transaction);
    } else {
      logger.info(`> executeQueryDirectly is started`);
    ...
    db.transaction(queryBuilderTrx => {
      Promise.all(
        queries.map(currentQuery => {
          ...
          if (beforeEventResult === false || isObject(beforeEventResult))
            return;
          if (!Array.isArray(query)) {
            throw new Error('query must be an array');
          }
...
module.exports.executeQueryDirectly = executeQueryDirectly;
