...
const releaseQuery = (queryName, dbName, originalQueryTemplate, groupId) => {
  if (queryName) {
    ...
    return db
      ...
      .first()
      .then(qft => {
        if (qft.query === originalQueryTemplate) {
          throw new Error(
        ...
        if (qft) {
          ...
          return db
            .insert({ name: queryName, query: originalQueryTemplate })
            .into('query_face_templates')
            .then(result => result);
        }
      });
...
const getQueryResult = function(req, res) {
