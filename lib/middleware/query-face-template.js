    ...
    if (isString(templateParamValue)) {
      ...
      if (paramPrefixMap[prefix]) {
        ...
        if (specialParamCalculatorMap[templateParamValue]) {
          ...
          if (match[1] && params[match[1]] !== undefined) {
...
const isValidQuery = (
  ...
  if (Array.isArray(query) && Array.isArray(queryTemplate)) {
    const copyQueryTemplate = [...queryTemplate];
    // INFO: if it is inner query, queryTemplate is not important
    if (isInnerQuery) {
      copyQueryTemplate.splice(0, copyQueryTemplate.length);
      query.forEach(q => copyQueryTemplate.push(q));
    }
    if (
      isInnerQuery ||
      query.length === copyQueryTemplate.length ||
      params ||
      query.length === 0
    ) {
      for (let i = 0; i < copyQueryTemplate.length; i++) {
        const currentTemplate = copyQueryTemplate[i];
